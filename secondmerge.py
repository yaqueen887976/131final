...
def second(new_data):
    ...
    '''
    new_data_year = pd.pivot_table(new_data, values='hrs1', columns=['year'], aggfunc=np.mean).T
    new_data_year['hrs1'].values
    ...
    model = sm.OLS(new_data_year['hrs1'], years_array)
    results = model.fit()
    print (results.params)

    plt.figure()
    ax = new_data_year['hrs1'].plot(marker='')
    xs = np.arange(1972, 2014)
    ax.set_ylabel(r"Mean hours/week") 
    ys = results.params['x1'] * xs + results.params['const']
    plt.plot(xs, ys, linewidth=1)
    plt.scatter(year, new_data_year['hrs1'])
    plt.title("Mean hours/week vs year")
    plt.show()
